function Card({ children, background, border, width, height, hideButton }) {
    ...
    return (
      <div
        className={` ${bg} ${uWidth} flex flex-col justify-center 
        items-center p-5 ${height} shadow-2xl shadow-indigo-900 
        ${uBorder} rounded-full`} style={{ borderRadius:hideButton ? '' : "2.2em" }}
      >
        {children}
      </div>
    );
